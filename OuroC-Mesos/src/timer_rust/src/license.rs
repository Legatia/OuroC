...
fn get_license_tier(api_key: &str) -> Option<LicenseTier> {
    ...
    // Enterprise license for OuroC-Mesos project
    if api_key == "ouro_enterprise_ouroc_mesos_2025" || api_key.starts_with("ent_") {
    ...
    if api_key == "ouro_community_shared_2025_demo_key" || api_key.starts_with("comm_") || api_key.starts_with("ouro_") {
        return Some(LicenseTier::Community);
    }

    // Beta tier
    if api_key.starts_with("beta_") {
        return Some(LicenseTier::Beta);
    }
