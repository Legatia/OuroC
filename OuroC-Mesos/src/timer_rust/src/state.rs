...
pub fn set_network(network: NetworkEnvironment) -> Result<(), String> {
    ...
    // Note: Solana uses Ed25519, so we use IC's Schnorr Ed25519 keys
    let (endpoint, _key_name) = match network {
        NetworkEnvironment::Mainnet => {
            ...
            ED25519_KEY_NAME.with(|k| *k.borrow_mut() = "key_1".to_string());
            ("https://api.mainnet-beta.solana.com", "key_1")
        }
