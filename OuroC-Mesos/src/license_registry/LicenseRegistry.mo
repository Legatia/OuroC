...
persistent actor LicenseRegistry {
    ...
    var developers: [(DeveloperId, Developer)] = [];
    var api_keys: [(ApiKey, ApiKeyInfo)] = [];
    ...
    public shared({caller = _}) func consume_license_usage(api_key: ApiKey): async Result.Result<(), Text> {
        ...
        switch (api_key_index) {
            ...
            case (?index) {
                ...
                let (new_remaining, new_reset) = if (now > info.rate_limit_reset) {
                    // Reset rate limit
                    (Nat.sub(get_rate_limit_for_tier(info.tier), 1), now + (60 * 60 * 1_000_000_000))
                } else if (info.rate_limit_remaining > 0) {
                    // Consume one usage
                    (Nat.sub(info.rate_limit_remaining, 1), info.rate_limit_reset)
                } else {
                    // Rate limit exceeded
    ...
    // Create or get the shared Community API key
    public shared({caller}) func ensure_community_api_key(): async Result.Result<ApiKey, Text> {
        if (not isAdmin(caller)) {
            return #err("Unauthorized: Admin access required");
        };

        // Check if Community API key already exists
        for ((key, info) in api_keys.vals()) {
            if (info.tier == #Community and Principal.equal(info.developer_id, Principal.fromText("aaaaa-aa"))) {
                return #ok(key);
            };
        };

        // Create shared Community API key
        let now = Time.now();
        let community_api_key = "ouro_community_shared_2025_demo_key";

        let api_key_info: ApiKeyInfo = {
            key = community_api_key;
            developer_id = Principal.fromText("aaaaa-aa"); // Anonymous principal for shared key
            tier = #Community;
            created_at = now;
            last_used = null;
            usage_count = 0;
            rate_limit_remaining = COMMUNITY_RATE_LIMIT;
            rate_limit_reset = now + (60 * 60 * 1_000_000_000); // 1 hour from now
        };

        api_keys := Array.append(api_keys, [(community_api_key, api_key_info)]);

        Debug.print("Created shared Community API key");
        #ok(community_api_key)
    };
