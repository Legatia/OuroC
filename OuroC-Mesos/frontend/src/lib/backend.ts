...
export async function createSubscription(
  ...
  try {
    ...
    console.log('📝 Would call Solana contract create_subscription with:', {
      subscriptionId,
    ...
    if (isRecurring) {
      ...
      if ("Ok" in result) {
        console.log(`✅ Timer canister subscription created: ${result.Ok}`);
        console.log(`🔔 Notifications: ${reminderDays > 0 ? 'Enabled (1 day before payment)' : 'Disabled'}`);
      ...
      console.log(`✅ One-time purchase created: ${subscriptionId}`);
      console.log(`⏭️ Skipped timer canister (one-time purchase)`);
...
export async function getPaymentSignature(
  ...
  try {
    ...
    if ("Ok" in result) {
      ...
      console.log(`✅ ICP signature generated for ${subscriptionId}`);
