...
pub fn create_subscription(
    ...
    // Validate interval: -1 for one-time, or >= 10 seconds for recurring
    if interval_seconds == -1 {
        // One-time purchase - valid
        msg!("Creating one-time subscription");
    } else if interval_seconds > 0 {
        // Recurring subscription - validate interval range
        require!(interval_seconds >= 10, ErrorCode::InvalidInterval); // Minimum 10 seconds for demo
        require!(interval_seconds <= 365 * 24 * 60 * 60, ErrorCode::InvalidInterval); // Maximum 1 year
        msg!("Creating recurring subscription with {} second interval", interval_seconds);
    } else {
        // interval_seconds == 0 or other negative values not allowed
    ...
    subscription.payment_token_mint = payment_token_mint;
    subscription.interval_seconds = interval_seconds;

    // Set next_payment_time: for one-time (-1), set to creation time; for recurring, add interval
    subscription.next_payment_time = if interval_seconds == -1 {
        clock.unix_timestamp  // One-time: immediate
    } else {
        clock.unix_timestamp + interval_seconds  // Recurring: first payment after interval
    };
    ...
    subscription.icp_canister_signature = None; // Signature not required at creation

    // Update global config
    ctx.accounts.config.total_subscriptions += 1;

    msg!(
        "Subscription created: {} for {} USDC, interval: {} seconds ({})",
        subscription.id,
        amount,
        interval_seconds,
        if interval_seconds == -1 { "one-time" } else { "recurring" }
    );
...
pub fn process_trigger(
    ...
    match opcode {
        ...
        1 => {
            ...
            // Note: ICP timer decides when to send notifications (1 day before for intervals > 1 day)
            let memo = format!(
                "{}: Payment reminder. Amount: {} USDC due soon",
                subscription.merchant_name,
                subscription.amount / 1_000_000  // Convert micro-USDC to USDC
            );
