...
module {
    ...
    public class SolanaClient(key_name: Text, rpc_config: SolanaRpcConfig) {
        ...
        // Extract balance from Solana RPC JSON response (no longer needed - kept for reference)
        // Response format: {"jsonrpc":"2.0","result":{"context":{"slot":123},"value":7500000000},"id":1}
        private func extract_balance_from_json(json: Text): ?Nat64 {
            // Simple parsing: find "value": and extract the number after it
            // We look for the pattern "value":<number>

            let value_key = "\"value\":";

            // Check if "value": exists in the JSON
            if (not Text.contains(json, #text value_key)) {
                return null;
            };

            // Split at "value": and take the second part
            let parts = Text.split(json, #text value_key);
            var after_value = "";
            var found = false;
            ...
            var num_text = "";
            for (char in after_value.chars()) {
                if (Char.isDigit(char)) {
                    num_text := num_text # Char.toText(char);
                } else if (num_text.size() > 0) {
                    // We've collected the number, stop
                    // (using a flag to avoid continue)
                    // Actually just exit the loop by checking later
                };
            };
            ...
            if (num_text.size() > 0) {
                ...
                for (char in num_text.chars()) {
                    let digit = Nat32.toNat64(Char.toNat32(char) - Char.toNat32('0'));
        ...
        private func get_recent_blockhash(): async Result.Result<Text, Text> {
            try {
                Debug.print("Chain Fusion: Querying latest blockhash via SOL RPC canister");

                // Use SOL RPC canister with default providers
                let rpc_sources: SolRpcTypes.RpcSources = #Default(_solana_cluster);
                let rpc_config_opt: ?SolRpcTypes.RpcConfig = null;

                let params: SolRpcTypes.GetLatestBlockhashParams = {
                    commitment = commitment_from_text(rpc_config.commitment);
                };

                // Call SOL RPC canister with cycles (10B cycles for HTTPS consensus)
                let result = await (with cycles = 10_000_000_000) sol_rpc.getLatestBlockhash(
                    rpc_sources,
                    rpc_config_opt,
                    params
                );
                ...
                switch (result) {
                    ...
                    case (#Err(error)) {
                        Debug.print("Chain Fusion RPC error: " # error.message);
                        #err("SOL RPC error: " # error.message)
                    };
                }
            } catch (error) {
                Debug.print("Chain Fusion RPC call exception caught");
                #err("Chain Fusion RPC call failed - check cycles balance and network")
            }
        };
        ...
        public func get_balance(address: SolanaAddress): async Result.Result<Nat64, Text> {
            try {
                Debug.print("Chain Fusion: Querying balance for " # address # " via SOL RPC canister");

                // Use SOL RPC canister with default Devnet providers
                let rpc_sources: SolRpcTypes.RpcSources = #Default(_solana_cluster);

                // Optional RPC config (use defaults for now)
                let rpc_config_opt: ?SolRpcTypes.RpcConfig = null;

                // Build params
                let params: SolRpcTypes.GetBalanceParams = {
                    address = address;
                    commitment = commitment_from_text(rpc_config.commitment);
                };

                // Call SOL RPC canister with cycles
                let result = await (with cycles = 2_000_000_000) sol_rpc.getBalance(
                    rpc_sources,
                    rpc_config_opt,
                    params
                );
                ...
                switch (result) {
                    case (#Ok(balance_result)) {
                        Debug.print("Chain Fusion: Got balance " # Nat64.toText(balance_result.value) # " lamports");
                        #ok(balance_result.value)
                    };
                    case (#Err(error)) {
                        #err("SOL RPC error: " # error.message)
                    };
        ...
        private func send_signed_transaction(transaction: ThresholdEd25519.SolanaTransaction, signature: ThresholdEd25519.Signature): async Result.Result<TransactionHash, Text> {
            try {
                ...
                let params: SolRpcTypes.SendTransactionParams = {
                ...
                let result = await (with cycles = 10_000_000_000) sol_rpc.sendTransaction(
                    rpc_sources,
                    rpc_config_opt,
                    params
                );
        ...
        public func get_usdc_balance(token_account: SolanaAddress): async Result.Result<Nat64, Text> {
            try {
                ...
                let params: SolRpcTypes.GetTokenAccountBalanceParams = {
                    token_account = token_account;
                    commitment = commitment_from_text(rpc_config.commitment);
                };

                let result = await (with cycles = 2_000_000_000) sol_rpc.getTokenAccountBalance(
                    rpc_sources,
                    rpc_config_opt,
                    params
                );
        ...
        public func get_all_token_balances(owner_address: SolanaAddress): async Result.Result<[{mint: Text; balance: Nat64; decimals: Nat8}], Text> {
            // NOTE: getTokenAccountsByOwner is not yet available in the SOL RPC canister
            // For now, return an empty array
            #ok([])
            /* TODO: Implement once SOL RPC canister adds getTokenAccountsByOwner
            try {
                let result = await (with cycles = 1_000_000_000) sol_rpc.getTokenAccountsByOwner({
                    owner = owner_address;
        ...
        public func withdraw_token(
            ...
            // NOTE: Token withdrawal requires getTokenAccountsByOwner which is not yet available
            return #err("Token withdrawal not yet available - waiting for SOL RPC canister getTokenAccountsByOwner support");

            /* TODO: Re-enable once getTokenAccountsByOwner is available
            // Select the correct keypair
            let keypair_option = switch (from_wallet) {
                case (#Main) main_keypair;
                case (#FeeCollection) {
